		...
		if (control == 's') {
			...
			while (true) {//
				...
				if (49 <= control || control <= 52) {
					...
					char buff[1];
					sprintf(buff, "%d", stage.getStageLevel());
					...
					/*
					attron(COLOR_PAIR(5));
					mvprintw(user.getYCoordinate(), user.getXCoordinate(), " ");
					...
					while (control != 'q') {
						...
						case 'w':
							...
							user.move(destinationX, destinationY, map[destinationX][destinationY], map);
							break;
						case 's':
							destinationX = user.getXCoordinate();
							destinationY = user.getYCoordinate() + 1;
						...
						mvwprintw(state, 4, 0, "push: ");
						char buff2[2];
						sprintf(buff2, "%d", user.getPush());
						mvwprintw(state, 4, 7, buff2);
						mvwprintw(state, 6, 0, "step: ");
						char buff3[2];
						sprintf(buff3, "%d", user.getStep());
